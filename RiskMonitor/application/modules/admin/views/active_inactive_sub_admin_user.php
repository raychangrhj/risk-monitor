...
<div id="page-wrapper">
	...
	<div class="row">
		<div class="col-lg-12">
			...
			<?php } ?>
			<form role='form' method='post' action="<?php echo base_url() . $this->config->item('admin_path') . 'user_type_changed/'; ?>" />
				<select class="form-control" name="user_type" style="width:200px">
					...
					echo '<option value=' . $this->config->item('user_level_sub_admin') . ($this->config->item('user_level_sub_admin')==$profile_setting_user_type?' selected':'') . '>Sub Admin</option>';
					echo '<option value=' . $this->config->item('user_level_user') . ($this->config->item('user_level_user')==$profile_setting_user_type?' selected':'') . '>User</option>';
					?>
				</select>
				...
				<div class="panel-body">
					...
					<div class="dataTable_wrapper">
						...
						<table class="table table-striped table-bordered table-hover">
							<?php $profile_setting_user_type=$this->session->userdata('profile_setting_user_type'); ?>
							<thead>
								<tr>
									<th>Company Name</th>
									<?php if(strcmp($profile_setting_user_type, $this->config->item('user_level_user'))==0){ ?>
									<th width="150">SubAdmin Name</th>
									<?php } ?>
							...
							<tbody>
								...
								<tr id="row-<?php echo $user_list[$i]->id; ?>">
									...
									<td class="right">
										<a href="<?php echo base_url() . $this->config->item('admin_path') . 'set_other_profile/' . $user_list[$i]->id; ?>" class="edit_btn"><i class="fa fa-pencil-square-o"></i></a>
