...
class Sub_admin extends CI_Controller {
	...
	function __construct()
	{
		...
		if($this->session->userdata('user_type') != $this->config->item('user_level_sub_admin')){
	...
	public function active_inactive()
	{
		...
		$this->load->view('active_inactive_user');
	}
	...
	{
		$this->session->set_userdata(array(
			'company_list' => $this->user_model->get_company_list_by_sub_admin_id($this->session->userdata('user_id'), $this->config->item('user_level_user'))
		));
